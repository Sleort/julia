...
mutable struct PostOptAnalysisState
    ...
    effect_free_if_argmem_only::Bool
    all_nothrow::Bool
    all_noub::Bool
...
function ((; code_cache)::GetNativeEscapeCache)(mi::MethodInstance)
    ...
    argescapes = codeinst.analysis_results
    return argescapes !== nothing ? argescapes::EscapeAnalysis.ArgEscapeCache : nothing
end
...
function refine_effects!(interp::AbstractInterpreter, sv::PostOptAnalysisState)
    if sv.all_effect_free && !isempty(sv.ea_analysis_pending)
        ...
        sv.result.analysis_results = EscapeAnalysis.ArgEscapeCache(estate)
...
function check_all_args_noescape!(sv::PostOptAnalysisState, ir::IRCode, stmt::Expr, estate::EscapeAnalysis.EscapeState)
    ...
    for arg in (isexpr(stmt, :invoke) ? stmt.args[2:end] : stmt.args)
        ...
        if isa(arg, Argument)
            ...
            # say is EFFECT_FREE_IF_INACCESSIBLEMEMONLY
        ...
        elseif isa(arg, SSAValue)
            ...
            argstate = estate[arg]
            has_no_escape(argstate) || return false
...
const IR_FLAGS_EA_REFINEMENT = IR_FLAG_EFIIMO | IR_FLAG_INACCESSIBLE_OR_ARGMEM
...
function scan_non_dataflow_flags!(inst::Instruction, sv::PostOptAnalysisState)
    ...
    need_ea_validation = (flag & IR_FLAGS_EA_REFINEMENT) == IR_FLAGS_EA_REFINEMENT
    if !need_ea_validation
        stmt = inst[:stmt]
